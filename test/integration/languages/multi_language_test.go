...
import (
	...
	"github.com/MacAttak/pi-scanner/pkg/testdata/languages"
...
func TestMultiLanguageTestCases(t *testing.T) {
	...
	runner := languages.NewMultiLanguageTestRunner(detector)
...
func testLanguage(t *testing.T, runner *languages.MultiLanguageTestRunner, language string) {
	testCases := runner.GetTestCasesByLanguage(language)
	require.NotEmpty(t, testCases, "Should have test cases for %s", language)

	ctx := context.Background()
	var results []*languages.MultiLanguageTestResult
	...
	summary := languages.GenerateSummary(results)
...
func TestMultiLanguageCodeConstructFiltering(t *testing.T) {
	...
	for _, caseID := range falsePositiveCases {
		t.Run(caseID, func(t *testing.T) {
			...
			var testCase *languages.MultiLanguageTestCase
...
func TestMultiLanguageContextFiltering(t *testing.T) {
	// Use MockContextDetector which actually implements context-based suppression
	baseDetector := detection.NewDetector()
	contextDetector := testutil.NewMockContextDetector(baseDetector)
	runner := languages.NewMultiLanguageTestRunner(contextDetector)
...
func TestMultiLanguageTestCaseValidation(t *testing.T) {
	runner := languages.NewMultiLanguageTestRunner(nil) // Don't need detector for validation
	allCases := runner.GetAllTestCases()
	...
	for _, testCase := range allCases {
		...
		supportedContexts := languages.GetSupportedContexts()
	...
	t.Logf("Validated %d test cases across %d languages",
		len(allCases), len(languages.GetSupportedLanguages()))
}

func TestMultiLanguageTestCoverage(t *testing.T) {
	runner := languages.NewMultiLanguageTestRunner(nil)

	// Ensure we have good coverage across languages and PI types
	for _, language := range languages.GetSupportedLanguages() {
